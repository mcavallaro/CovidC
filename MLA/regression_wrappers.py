...
def survival_cox(data, ax=None, duration_name='interval', event_name='death', penalizer=0, title=None):
#     from coxph_fitter import CoxPHFitter
    from lifelines import CoxPHFitter
    import matplotlib.pyplot as plt
    cph = CoxPHFitter(penalizer=penalizer)
    cph.fit(data, duration_col=duration_name, event_col=event_name)
    if ax != None:
        plt.sca(ax)

## Uncomment this to plot without ordering
#         columns = [c for c in data.columns if c not in [duration_name, event_name]]
#         print(columns)
#         cph.plot(columns=columns, ax=ax)

        cph.plot(ax=ax)
        ax.set_title(title)
#         plt.sca(ax)
#     return cph.summary[['coef', 'se(coef)', 'z', 'p', 'lower 0.95', 'upper 0.95']]
    return cph

def survival_na(duration, observed, ax, label=None, ax1=None):
    from lifelines import NelsonAalenFitter
    naf = NelsonAalenFitter()
    naf.fit(duration, event_observed=observed, label=label)
    naf.plot(ax=ax)
    if ax1 != None:
        naf.plot_hazard(ax=ax1, bandwidth=50)
        
        
def survival_km(duration, observed, ax, label=None):
    from lifelines import KaplanMeierFitter
    kmf = KaplanMeierFitter()
    kmf.fit(duration, event_observed=observed, label=label)
    kmf.plot(ax=ax)
